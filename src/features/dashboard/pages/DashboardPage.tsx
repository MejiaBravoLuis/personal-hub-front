...
import { useConnectSpotify } from '@/features/spotify/hooks/useSpotify'
...
export function DashboardPage() {
  ...
  const connectInstagram = useConnectProvider('instagram')
  const connectCanvas = useConnectProvider('canvas')

  return (
    <div className="space-y-8">
      <PageHeader
        title="Tu plataforma"
        description="Los módulos externos se muestran censurados hasta que conectes su API."
        actions={<Badge variant="accent">Hub en vivo</Badge>}
      />
      ...
      >
        <CensoredModule
          locked={locks.spotify}
          loading={isLoading}
          title="Spotify bloqueado"
          description="Conecta Spotify para ver tu reproducción real."
          actionLabel="Conectar Spotify"
          actionLoading={connectSpotify.isPending}
          onAction={() => connectSpotify.mutate()}
          ...
          >
            ...
            <div className="relative flex h-full flex-col gap-5">
              ...
              <div className="mt-auto flex items-center gap-2">
                ...
                <Badge variant={locks.spotify ? 'default' : 'accent'}>
                  {locks.spotify ? 'Bloqueado' : 'Conectado'}
                </Badge>
              </div>
            </div>
          </motion.article>
        ...
        <CensoredModule
          ...
          description="Conecta Instagram para desbloquear mensajes y feed."
          actionLabel="Conectar Instagram"
          actionLoading={connectInstagram.isPending}
          onAction={() => connectInstagram.mutate()}
      ...
      >
        ...
        >
          ...
          <p className="mt-4 text-xs text-[var(--foreground-subtle)]">
            Vista previa. Los todos reales viven en{' '}
            <Link to={ROUTES.todos} className="underline underline-offset-2">
              Todos
            </Link>
            .
          </p>
        ...
        <CensoredModule
          ...
          description="Conecta Canvas para ver cursos y entregas reales."
          actionLabel="Conectar Canvas"
          actionLoading={connectCanvas.isPending}
          onAction={() => connectCanvas.mutate()}
